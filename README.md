...
Profile discovery uses `gallery-dl` in metadata-only mode and returns at most 24 items per request. Instagram discovery includes posts and reels, TikTok includes posts, and Facebook includes profile photos. When more media is available, an opaque continuation cursor lets the user load and append the next bounded page without imposing a total browsing limit. Up to 100 selected items are accepted in one batch API request and their canonical post URLs are queued through the same collection pipeline and limits as manually pasted links.
