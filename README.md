...
`POST /profile-archives` starts a background archive and enables automatic collection in one request. The archive follows the profile continuation cursor one bounded page at a time until all available media has been discovered, persists its progress in PostgreSQL, and resumes queued or interrupted runs when the worker starts. Archives and scheduled checks share the same deduplication path, so they can safely overlap. Stories mean stories that are currently available; expired stories cannot be archived.
